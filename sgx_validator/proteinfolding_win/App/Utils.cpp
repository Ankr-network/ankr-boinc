    ...
    for (idx = 0; idx < ttl; idx++) {
        if(ret == sgx_errlist[idx].err) {
            if(NULL != sgx_errlist[idx].sug)
                LL_CRITICAL("Info: %s", sgx_errlist[idx].sug);
            LL_CRITICAL("Error: %s", sgx_errlist[idx].msg);
            break;
        }
    }
...
/*
    ...
    [return] len: how many bytes get written to the target
*/
int fromHex(const char* src, uint8_t* target)
{
    int len = 0;
    if ((strlen(src) % 2) != 0)
