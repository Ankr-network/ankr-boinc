...
long pouw_main (pow_spec* spec, output_t* output, char* enclave_output, int len);
...
void protain_folding () {
    ...
    geneticAlgorithm.run(enclave_output, 4096);
}

long pouw_main (pow_spec* spec, output_t* output, char* enclave_output, int len) {
    int ret = 0;
    GeneticAlgorithm geneticAlgorithm;
    geneticAlgorithm.run(enclave_output, len);
    return ret;
}
