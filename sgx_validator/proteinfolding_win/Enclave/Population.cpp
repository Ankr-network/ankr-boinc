...
Population Population::tournament_selection(int generation) {
 ...
 //   Chromosome* current = (chromosomes.begin());
	std::vector<Chromosome>::iterator current = chromosomes.begin();
    while (t_rounds) {
		std::vector<Chromosome>::iterator rI1 = current;
		std::vector<Chromosome>::iterator rI2 = current;
...
void Population::crossover_selection(double crossover_rate) {
    ...
    while (crossover_count) {
		std::vector<Chromosome>::iterator r = current;
...
void Population::printBestCandidate(char* enclave_output, int len) {
    std::sort(chromosomes.rbegin(), chromosomes.rend());
    chromosomes.front().printInfo(enclave_output, len);
}
