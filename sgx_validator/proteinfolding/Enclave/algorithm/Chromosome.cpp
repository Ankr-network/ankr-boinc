...
void Chromosome::printCoordinates(char* output, int len) {
//    for (Coordinate coordinate : pathList){
    ...
    for (unsigned i = 0; i < pathList.size(); i++) {
        ...
        snprintf(start, len-(int)(start - output),"%s", coordinate.representation().c_str());
        start += strlen(start);
    }
    printf_sgx("\n");
    snprintf(start, len-(int)(start - output), "\n");
    start += 1; 
}

void Chromosome::printTurns(char* output, int len) {
...
//    for (TURNCODE turn: turnList){
    ...
    for (unsigned i = 0; i < turnList.size(); i++) {
        TURNCODE turn = turnList.at(i);
        printf_sgx("%d ", turn);
        snprintf(start, len-(int)(start - output),"%d ", turn);
        start += 2;
    }
    printf_sgx("\n");
    snprintf(start, len-(int)(start - output), "\n");
...
void Chromosome::printInfo(char* enclave_output, int len) {

    char* start = enclave_output;
    printf_sgx("### Best Candidate ###\n");
    snprintf(start, len-(int)(start - enclave_output), "### Best Candidate ###\n");

    start += strlen(start);
    printf_sgx("Pairs: %d\n", pairs.size());
    snprintf(start, len-(int)(start - enclave_output), "Pairs: %d\n", pairs.size());
    start += strlen(start);
    printf_sgx("Collisions: %d\n", collisions);
    snprintf(start, len-(int)(start - enclave_output), "Collisions: %d\n", collisions);
    start += strlen(start);
    printf_sgx("Fitness: %f\n", fitness);
    snprintf(start, len-(int)(start - enclave_output), "Fitness: %f\n", fitness);
    start += strlen(start);
    printf_sgx("ID: %d\n",id);
    snprintf(start, len-(int)(start - enclave_output), "ID: %d\n",id);
    start += strlen(start);

    printTurns(start, len-(int)(start - enclave_output));
    start += strlen(start);
    printCoordinates(start, 0);
    start += strlen(start);
